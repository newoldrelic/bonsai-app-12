...
export function MaintenanceSection({ 
  ...
  const handleConsentConfirm = async () => {
    if (pendingToggleType) {
      onNotificationChange(pendingToggleType, true);
      setError(null);
    }
  ...
  const handleNotificationToggle = async (type: keyof NotificationPreferences, enabled: boolean) => {
    try {
      debug.info('Notification toggle requested', { type, enabled });
      ...
      // Just update the state - actual scheduling will happen in the parent form
      onNotificationChange(type, enabled);
      setError(null);
